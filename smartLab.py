...
def saveDataToFileCsv(data):
	with open('TraMyDinh.csv', mode='w') as TraMyDinh :
		TraMyDinh= csv.writer(TraMyDinh, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
...
def checkPointWithTable(table , C , img ):
	for i in range(7):
		check = cal.checkCondition( (cal.distanceAH(table[i] , cal.solvePointH(table[i],C))) , (cal.distanceBH(table[i] , cal.solvePointH(table[i],C))), cal.distanceAB(table[i]), (cal.distanceFromCtoAB(table[i],C)) ,de.distanceValue)
		if check == True :
			print ("OK OK OK OK")
	...
	while 1:
		...
		for i in range(len(boxes)):
			...
			if label == 'people':
				...
				checkPointWithTable(de.TABLE,creatCenterPoint(x, y, w, h, img), img)
		...
		for i in  range(7) :
			cv2.putText(img, "Table" + str(i+1) + ": " + str(people[i]) + " people",
						(de.TABLE[i][0][0] - 100, de.TABLE[i][0][1] - 150), font, 2, (0, 255, 255), 4)
